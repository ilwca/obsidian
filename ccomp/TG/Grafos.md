...
Dado um grafo $G(V,E)$ conexo, este algoritmo gera um $T(V,E_t)$ onde $E_t$ é o conjunto de aresta da arvore $T$. O algoritmo divide o conjunto das arestas $E$ em dois grupos, sendo o grupo ($I$) das arestas visitadas no algoritmo _(arestas retas)_, e o grupo ($II$) das arestas de retorno, que são as arestas não visitadas pelo algoritmo mas que estão presentes em $E(G)$ _(As arestas curvas)_ denominadas como frondes ou aresta de retorno . Como vistas na imagem abaixo:
...
A busca de profundidade em digrafos, divide o conjunto de arestas em 4 conjuntos disjuntos. Onde temos:
### Aresta de Arvore
...
Caso $w$ seja alcançado antes de $v$ na busca, se $w \notin Q$, ou seja, fora da pilha no momento da visita, então $(v,w)$ e chamada aresta de retorno. Pois a aresta foi explorada no momento em que $w$ ja saiu de pilha ou seja toda a sub-arvore que $w$ esta incluso já foi explorada.

# Busca em Largura
Assim como no conceito geral da busca, que não estabelece critério de seleção do vértice fomos apresentados a busca em profundidade onde é estabelecido um critério para a seleção dos vértices explorados. De maneira análoga, a busca em largura também estabelece os critérios para a busca dos vértice, que é quando:
	_"Dentre todos os vértices marcados e incidentes a alguma aresta, escolher aquele que foi marcado menos recente."_
Ou seja, para o auxilio da busca ao invés de usarmos uma pilha como na busca em profundidade, utilizaremos uma fila.
## Arestas de Busca em Largura
Os teoremas usados na busca em largura normalmente dividem o conjunto de $E$ de arestas em duas partes. Arestas $(v,w)$ em que $|nivel(v)-nivel(w)|=1$ e em arestas que $|nivel(v)-nivel(w)|=1$.
### Aresta Pai
Arestas $(v,w)$ em que $v$ é pai de $w$ (ou arestas de arvore)em $T$.
### Aresta Tio
Arestas $(v,w)$ é tio quando $nivel(w)=nivel(v)+1$ e $(v,w)\notin E_T$.
### Aresta irmão
